...
/*   Updated: 2024/08/09 04:24:07 by yublee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_ast	*parse_redirection_out(t_list *token_list)
{
	...
	while (current && token->type != TK_PIPE)
	{
		...
		{
			tree_node = ast_new_node((t_token *)current->next->content);
			tree_node->left = NULL;
			tree_node->right = parse_redirection_out(current->next);
			return (tree_node);
		}
		current = current->next;
	}
...
static t_ast	*parse_redirection_in(t_list *token_list)
{
	...
	while (current && token->type != TK_PIPE)
	{
		...
		{
			tree_node = ast_new_node((t_token *)current->next->content);
			tree_node->left = parse_redirection_in(current->next);
